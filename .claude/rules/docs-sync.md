...
| Changed | Update |
|---------|--------|
| License (`package.json` / `LICENSE`) | README badges + License section, `llms.txt`, any `specs/`/`docs/` mention |
| Public API (client methods, args, returns, error codes) | README usage + API table, `docs/API.md`, `llms.txt` |
...
| New file / capability / `./react` entry | README, `llms.txt` index |

## Verify before commit (BLOCKING)

1. **Grep the OLD value** you changed across the repo and confirm zero stale hits:
...
2. Trace test: every doc claim still matches the code. A doc that describes
...
but left `Apache-2.0` in README, `llms.txt`, and a `specs/` file —
a visible, shipped inconsistency. Step 1 (grep the old value) catches exactly this.
